...
- Postgres 成为完整性关键组件:M3 之前必须写出备份与恢复流程。**已交付**:`docs/runbook/backup-recovery.md` + `scripts/pg-{backup,restore}.sh` + `scripts/verify-backup-integrity.sh`(完整 round-trip 校验,跑 `agent-lens-hook verify` 确认 hash chain),Make targets `db-backup` / `db-restore` / `db-verify-backup`。
