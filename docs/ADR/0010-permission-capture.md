...
- 修订(待 Accepted):SPEC §10.1;ADR 0004 D1 的 `decision` 值集合增 `unresolved`(见 § 后果)
...
- 重放幂等 + 顺序:`permission_decision` 的 outcome 在 `PostToolUse`/`PostToolUseFailure` 配对或 Stop 收尾时判定,故晚于它链回的 `tool_call` 入链——正常因果方向(同 ADR 0005 D7),verify 时按 `target_event_id` 校验。hook transport at-least-once 通道可能重复投递,可按 (session_id, target_event_id) 去重。**注意**:`human_intervention` EventKind 当前**尚未落地**(0004 Accepted-but-unlanded),该去重机制须在 0004 的落地 PR 中实际构建,本 ADR 依赖它、不可假定已存在。
