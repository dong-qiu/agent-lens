...
- 重放幂等:`PreCompact` / `PostCompact` 经 hook transport 的 at-least-once 通道(NDJSON fallback + `replay`)可能重复投递,与既有 hook 事件通性一致;compaction 事件带 `applied_at` + `trigger` + 时间窗,可在 linker 侧按 (session_id, applied_at) 去重。**注意**:`context_transform` EventKind 当前**尚未落地**(0005 Accepted-but-unlanded),该去重机制须在 0005 的落地 PR 中实际构建,本 ADR 的 hook 事件依赖它,不可假定已存在。
