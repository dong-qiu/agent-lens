...
- **0014 用每事件幂等键去重,与 id / 哈希链排序解耦**(草案):#81——hook 不设 id 致重放重复;排序 / 链头都靠服务端 ULID(完整性载荷),故 `id` 不动、另加**每事件 ULID** `idempotency_key` 做 dedup(`ON CONFLICT DO NOTHING` + 真实 inserted 数;顺带修 deploy/github 把外部 id 塞进 `id` 的既有违反)。
